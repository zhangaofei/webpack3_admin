...
<script>
    ...
    import {grouplistForAdd, listForEdit, getselectedPageList} from "api/enWAS/group";
    export default {
        ...
        methods: {
            ...
            changeStatus(){
                this.$emit("onDomainStatus")
            },
            getSelectedList(){
                this.listQuery.group_id = this.id;
                getselectedPageList(this.listQuery).then(response => {
                    this.listQuery.total = response.data.black.page_count;
                    this.selectedList = response.data.black.res;
//                    for (let i = 0; i < response.data.black.res.length; i++){
//                        this.selectedList.push(response.data.black.res[i]);
//                    }
                }).catch((e) => {
                    console.log(e);
                })
