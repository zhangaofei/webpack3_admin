    ...
    <div class="admin public">
        <el-dialog title="选择应用关联用户组" :visible.sync="addVisible" @close="handleClose" size="small" custom-class="dialog public" top="10%">
            ...
            <div class='form-footer ' slot="footer" >
                <div class="foot">
                    <span class="cancle" @click="sourceReset">取消</span>
                    <el-button
                            type="primary"
...
<script>
    ...
    export default {
        ...
        methods: {
            getList(){
                grouplistForAdd(this.listQuery).then(response => {
                   if (response.data.items2.page_count > 0){
                       ...
                       this.allSelectedResource = this.cancelResource;
                       let selectedIndexes = [];
                       for (let i = 0; i < this.resourceList.length; i++){
                           for (let j = 0; j < this.allSelectedResource.length; j++){
                               if (this.resourceList[i].id == this.allSelectedResource[j].id){
            ...
            handleClose() {
                this.$emit("onResourceStatus", this.cancelSelected);
            },
            doTest() {
            ...
            sourceSet(formName) {
                this.addVisible = false;
                this.$emit("onResourceStatus",this.allSelectedResource);
            }
        }
    }
</script>
