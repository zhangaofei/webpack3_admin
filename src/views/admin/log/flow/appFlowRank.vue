            ...
            <div style="width: 100%;min-height: 500px;">
                <div class="log-chart" style="height: 520px;width:40%;min-width:524px;">
                    ...
                    <div style=";height: 500px">
                        <barChart :data="barData" :keyName="barName" id="appFlowBar"></barChart>
                    </div>
                </div>
                <div class="log-chart log-pie-chart" style="height: 520px;width: 40%;min-width:490px;">
                    <pieChart :data="pieData" :legend="legend" title="应用资源流量占比" id="appFlowPie"></pieChart>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    import {trafficPie, trafficRank} from "../../../../api/log/logApi";
    ...
    export default {
        ...
        watch: {//深度 watcher
            "topValue": "getResourceTrafficRank"
        },
        methods: {
            changeStartTime(e){
            ...
            search() {
                this.getResourceTrafficRank();
                this.getResourceTrafficPie();
            ...
            // 获取登陆排行的直方图数据
            getResourceTrafficRank() {
                var choice = '';
                trafficRank("resource", this.topValue, this.timeCondition).then(resp => {
                    ...
                    console.log("获取资源流量排行的数据出错：", err)
                });
            },
            //获取用户组流量占比
            getResourceTrafficPie() {
                trafficRank("resource", this.topValue, this.timeCondition).then(resp => {
                    let legend = [];
                    let data = [];
                    resp.data.xAxis.forEach(function (x, index) {
                        legend.push(x);
                        data.push({
                            value: resp.data.yAxis[index],
                            name: x
                        })
                    })
                    this.legend = legend;
                    ...
                    console.log("获取访问资源的类型占比数据出错：", err)
                })
            }
        }
    }
