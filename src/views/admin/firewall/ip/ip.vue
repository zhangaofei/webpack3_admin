                ...
                <div class="col-md-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title" id="res_rdp_title">IP设置</h3>
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <!-- 表单搜索框部分 -->
...
<script>
    ...
    import {getIpList, deleteIp} from 'api/firewall/ip';
    ...
    export default {
        ...
        methods: {
            ...
            getList(keyword) {
                this.listQuery.searchContent = keyword != undefined ? keyword : this.listQuery.searchContent;
                getIpList(this.listQuery).then(response => {
            ...
            onDelete(data) {
                if(this.readOnly == true){
                    ...
                    this.$confirm('确认删除?', '提示', {
                        ...
                        deleteIp(ipDto).then(response => {
                            if (response.status == 'SUCCESS') {
                                if (response.data.indexOf("-") > -1) {
                                    var name = response.data.split("-")[1];
                                    this.$message({
                                        type: 'warning',
                                        message: '不可删除! 名称为 ' + name + " 的ip设置已被引用"
                                    });
                                } else {
                                    this.$message({
                                        type: 'success',
            ...
            ipAddressFormatter(row, column) {
                if (row.rangeMask==0){
                    return row.ip+"-"+row.ipEnd;
                }else if (row.rangeMask==1){
                    return row.ip+"/"+row.ethNum;
                }else {
                    return row.ip;
                }
