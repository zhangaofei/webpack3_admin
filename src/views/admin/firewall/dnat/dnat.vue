                ...
                <div class="col-md-12">
                    <div class="box" >
                        <div class="box-header">
                            <h3 class="box-title" id="res_rdp_title">目的地址转换</h3>
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <!-- 表单搜索框部分 -->
                            ...
                            <el-table
                                ...
                                <el-table-column
                                        align="center"
                                        label="公开ip"
                                        :formatter="pubIpFormatter">
                                </el-table-column>
                                <el-table-column
                                        align="center"
                                        label="内部ip"
                                        :formatter="innerIpFormatter">
                                </el-table-column>
                                <el-table-column
                                        prop="outServiceInfo.name"
                                        align="center"
                                        label="对外服务"
                                        >
                                </el-table-column>
                                <el-table-column
                                        prop="inServiceInfo.name"
                                        align="center"
                                        label="对内服务"
                                        >
                                </el-table-column>
                                <el-table-column
                                    ...
                                    <template slot-scope="scope">
                                        {{scope.row.ethIn=="all"?"任何":scope.row.ethIn}}
                                    </template>
                                </el-table-column>
...
<script>
    ...
    import {getDnatList, deleteDnat,sortDnat} from 'api/firewall/dnat';
    ...
    export default {
        ...
        methods: {
            ...
            getList(keyword) {
                this.listQuery.searchContent = keyword != undefined ? keyword : this.listQuery.searchContent;
                getDnatList(this.listQuery).then(response => {
            ...
            onDelete(data) {
                if(this.readOnly == true){
                    ...
                    this.$confirm('确认删除?', '提示', {
                        ...
                        deleteDnat(fwtimeDto).then(response => {
                            if (response.status == 'SUCCESS') {
                                this.$message({
                                    type: 'success',
                                    message: '删除成功!'
                ...
                if(this.readOnly == true){
                    ...
                    if (this.checkedItems.length == 0) { // 如果没有一项被选中则弹窗提示
                        ...
                        if (row.state == '0') {
                            ...
                            sortDnat(dto).then(response => {
                                if (response.status == 'SUCCESS') {
                                    if (response.data == "up-already") {
                                        this.$message({
                                            type: 'warning',
                                            message: '当前目的地址转换已经置顶!'
                                        });
                                    } else if (response.data == "down-already") {
                                        this.$message({
                                            type: 'warning',
                                            message: '当前目的地址转换已经置底!'
                                        });
                                    } else {
                                        this.$message({
                                            type: 'success',
                ...
                if(this.readOnly == true){
                    ...
                    if (this.checkedItems.length == 0) { // 如果没有一项被选中则弹窗提示
                        ...
                        if (row.state == '0') {
                            ...
                            sortDnat({id: row.id, direction: "down"}).then(response => {
                                if (response.status == 'SUCCESS') {
                                    if (response.data == "up-already") {
                                        this.$message({
                                            type: 'warning',
...
//                            });
            ...
            pubIpFormatter(row, column) {


                if(row.pubIpType=="1"){//自定义
                    return row.pubIp;
                }else {//预定义
                    if (row.pubIpType=="0"){
                        return row.pubIpInfo.name;
                    }else {
                        return row.pubIpgInfo.name;
                    }
                }
            ...
            innerIpFormatter(row, column) {

                if(row.innerIpType=="1"){//自定义
                    return row.innerIp;
                }else {//预定义
                    if (row.innerIpType=="0"){
                        return row.innerIpInfo.name;
                    }else {
                        return row.innerIpgInfo.name;
                    }
                }
