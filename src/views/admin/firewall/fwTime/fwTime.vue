                ...
                <div class="col-md-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title" id="res_rdp_title">时间设置</h3>
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <!-- 表单搜索框部分 -->
...
<script>
    ...
    import {getTimeList, deleteTime} from 'api/firewall/fwTime';
    ...
    export default {
        ...
        methods: {
            ...
            getList(keyword) {
                this.listQuery.searchContent = keyword != undefined ? keyword : this.listQuery.searchContent;
                getTimeList(this.listQuery).then(response => {
            ...
            onDelete(data) {
                if(this.readOnly == true){
                    ...
                    this.$confirm('确认删除?', '提示', {
                        ...
                        deleteTime(fwtimeDto).then(response => {
                            if (response.status == 'SUCCESS') {
                                if (response.data.indexOf("-") > -1) {
                                    var name = response.data.split("-")[1];
                                    this.$message({
                                        type: 'warning',
                                        message: '不可删除! 名称为 ' + name + " 的时间设置已被引用"
                                    });
                                } else {
                                    this.$message({
                                        type: 'success',
            ...
            weekFormatter(row, column) {
                var day = row.weekday.split(",");
                if (day.length<1){
                    return "";
                }else {
                    var isWeekday = day.indexOf("Mon")>-1&&day.indexOf("Tue")>-1&&day.indexOf("Wed")>-1&&day.indexOf("Thu")>-1&&day.indexOf("Fri")>-1;
                    var isWeekend = day.indexOf("Sat")>-1&&day.indexOf("Sun")>-1;
                    var show=[];


                    if (isWeekend==true&&isWeekday==true){
                        show.push("每天");
                    }else {
                        if(isWeekday){
                            show.push("工作日");
