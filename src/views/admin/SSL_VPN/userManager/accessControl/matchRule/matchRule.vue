                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            ...
                            <el-table
                                ...
                                <el-table-column  label="匹配条件" align="center" show-overflow-tooltip>
                                    <template slot-scope="scope">
                                        {{scope.row.rule}}
                                        <!--{{scope.row.ruleList|filtersRuleList}}-->
                                    </template>
                                </el-table-column>
...
<script>
    ...
    import {getAclBasicRule,delAclBasicRule} from "@/api/SSL_VPN/accessControl/accessControl";
    export default{
        components: {
            add,
            edit,
        ...
        filters:{
           // 数组两边的引号报错
           /* filtersRuleList(list){
                let showList=[];
                if(list){
                    for(let item of JSON.parse(list)){
                        showList+=item.type+item.regOption+item.regVal+';'
                    }
                    return showList
                }
            }*/
        ...
        methods: {
           /* transfrom(list){
            ...
            delete(deleteDTO){
                delAclBasicRule(deleteDTO).then((resp) => {
                    if (resp.status == 1) {
                        this.$message({
                            type: 'success',
                            message: '删除匹配规则成功！'
            ...
            getList(keyword) {
                this.listQuery.searchContent = keyword != undefined ? keyword : this.listQuery.searchContent;
                this.list = [];
                getAclBasicRule().then(resp => {
                    // this.listQuery.total = resp.extend.page.totalHits;
                    this.list = resp.data;
                   /* for(let item of resp.data){
                        for(let key in item){
                            item.rule=item[key].operator+item[key].value
                        }
                    }*/
                    }).catch((e) => {
                    console.log(e)
