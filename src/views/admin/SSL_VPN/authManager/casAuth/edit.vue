        ...
        <el-dialog title="编辑cas认证"
            ...
            <el-form :model="info" label-width="193px" :rules="rules" class="OAuth-form" ref="info">
                <el-form-item label="cas server地址" prop="cas_server" v-if="currentData.name=='cas_server'">
                    <el-input v-model.trim="info.cas_server" class="el_put" placeholder="示例：https://cas.enlink.cn:9990"></el-input>
                </el-form-item>

                <el-form-item label="cas协议登录uri" prop="cas_login" v-if="currentData.name=='cas_login'">
                    <el-input  class="el_put" v-model.trim="info.cas_login" placeholder="示例：/cas/login"></el-input>
                </el-form-item>
                <el-form-item label="cas协议获取用户信息uri" prop="cas_service_validate" v-if="currentData.name=='cas_service_validate'">
                    <el-input v-model.trim="info.cas_service_validate" class="el_put" placeholder="示例：/cas/serviceValidate"></el-input>
                </el-form-item>

                <el-form-item label="oauth协议登录uri" prop="oauth_authorization" v-if="currentData.name=='oauth_authorization'">
                    <el-input v-model.trim="info.oauth_authorization" class="el_put" placeholder="示例：/cas/oauth2.0/authorize"></el-input>
                </el-form-item>
                <el-form-item label="oauth协议获取访问令牌uri" prop="oauth_access_token" v-if="currentData.name=='oauth_access_token'">
                    <el-input v-model.trim="info.oauth_access_token" class="el_put" placeholder="示例：/cas/oauth2.0/accessToken"></el-input>
                </el-form-item>
                <el-form-item label="oauth协议获取用户信息uri" prop="oauth_profile" v-if="currentData.name=='oauth_profile'">
                    <el-input v-model.trim="info.oauth_profile" class="el_put" placeholder="示例：/cas/oauth2.0/profile"></el-input>
                </el-form-item>

                <el-form-item label="oauth2.0认证client_id" prop="APP_KEY" v-if="currentData.name=='APP_KEY'">
                    <el-input v-model.trim="info.APP_KEY" class="el_put" placeholder="oauth2.0认证client_id"></el-input>
                </el-form-item>
                <el-form-item label="oauth2.0认证密钥" prop="APP_SECRET" v-if="currentData.name=='APP_SECRET'">
                    <el-input v-model.trim="info.APP_SECRET" class="el_put" placeholder="oauth2.0认证密钥"></el-input>
                </el-form-item>
            </el-form>
        ...
        methods: {
            ...
            submitInfo(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        EditCasAuth(this.info).then(resp => {
                            if (resp.status == 1) {
                                this.$message({
                                    type: 'success',
                                    message: '编辑成功！'
